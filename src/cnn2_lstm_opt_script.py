...
class CNN2_LSTMnn(nn.Module):
    ...
    def __init__(
        ...
        self.bn = nn.BatchNorm1d(conv2_out_channels)
        self.relu = nn.ReLU()
        self.lstm = nn.LSTM(conv2_out_channels, hidden_size, num_layers, batch_first=True, dropout=dropout if num_layers>1 else 0.0)
...
def objective(trial):
    ...
    kernel_size2 = trial.suggest_int('kernel_size2', 2, 7)
...
study = optuna.create_study(
    ...
    study_name='cnn2_lstm_opt_tpe_hyperband_gpu'
)
study.optimize(objective, n_trials=400, timeout=60*60*24*7-60*60)
best_trial = study.best_trial
joblib.dump(study, os.path.join(SCRATCH_PATH, 'cnn2_lstm_tpe_hyperband_gpu.pkl'))
joblib.dump(best_trial, os.path.join(SCRATCH_PATH, 'cnn2_lstm.pkl'))


# #!/bin/bash
# #SBATCH --job-name=cnn2_lstm_optuna
# #SBATCH --output=/scratch/%u/cnn2_lstm_optuna_%j.out
# #SBATCH --error=/scratch/%u/cnn2_lstm_optuna_%j.err
# #SBATCH --account=stat
...
# srun python /u/bleidig2/cnn2_lstm_opt_script.py