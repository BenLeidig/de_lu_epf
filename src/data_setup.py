...
def save_splits(
    ...
    for (train_start, train_end, test_start, test_end), (f_train, f_test) in zip(
        time_splits, f_names
    ):
        ...
        df_train_scaled = pd.DataFrame(
            ss.fit_transform(df_train), columns=df_train.columns
        )
...
def save_imf_splits(
    ...
    for (train_start, train_end, test_start, test_end), (f_train, f_test) in zip(
        ...
        df_train_imf = pd.DataFrame(
            data=np_train_imf, columns=[f"imf{i}" for i in range(1, 6)]
        )
        ...
        df_train_imf_scaled = pd.DataFrame(
            data=np_train_imf_scaled, columns=df_train_imf.columns
        )
        df_train_imf_scaled = df_train_imf_scaled.set_index(df_train.index)

        # VMD test
        np_test_imf = vmd.transform(np_test_price)
        df_test_imf = pd.DataFrame(
            data=np_test_imf, columns=[f"imf{i}" for i in range(1, 6)]
        )
        ...
        df_test_imf_scaled = pd.DataFrame(
            data=np_test_imf_scaled, columns=df_test_imf.columns
        )
